...
class FarmRemoteDatasource {
  ...
  static const String _path = '/api/v1/farms';

  final http.Client _client;
  FarmRemoteDatasource({http.Client? client})
  ...
  Future<http.Response> getById(int farmId) async {
    final uri = Uri.parse('$_base$_path/$farmId');
    return _client.get(uri, headers: await _headers());
  }

  Future<http.Response> getZonesByFarm(int farmId) async {
    final uri = Uri.parse('$_base$_path/$farmId/zones');
    return _client.get(uri, headers: await _headers());
  }
}